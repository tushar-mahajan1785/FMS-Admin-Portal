            ...
            <Grid size={{ xs: 12, sm: 12, md: 4, lg: 4, xl: 4 }}>
                ...
                <Card sx={{ borderRadius: '16px', padding: '24px', gap: '32px', border: `1px solid ${theme.palette.grey[300]}`, mt: 2 }}>
                    <CardContent sx={{ p: 0 }}>
                        ...
                        />
                        <Stack sx={{ flexDirection: 'row', columnGap: 2, my: 2 }}>
                            {
                                assetTypeMasterOption !== null && assetTypeMasterOption.length > 0 &&
                                assetTypeMasterOption.map((asset, index) => (
                                    <Button
                                        key={index}
                                        sx={{
                                            ...
                                            px: 6,
                                            borderRadius: '8px',
                                            backgroundColor: asset?.name === rosterData?.asset_type ? theme.palette.primary[600] : '',
                                            color: asset?.name === rosterData?.asset_type ? theme.palette.common.white : theme.palette.grey[700],
                                            fontSize: 16,
                                            fontWeight: 600,
                                            borderColor: asset?.name === rosterData?.asset_type ? theme.palette.primary[600] : theme.palette.grey[300]
                                        }}
                                        variant={asset?.name === rosterData?.asset_type ? 'contained' : 'outlined'}
                                        onClick={() => {
                                            let objData = Object.assign({}, rosterData)
                                            objData.asset_type = asset?.name
                                            dispatch(actionRosterData(objData))
                                        }}
                                    >
                                        {asset?.name}
                                    ...
                                    <Controller
                                        ...
                                        render={({ field }) => (
                                            <CustomTextField
                                                fullWidth
                                                value={field?.value}
                                                label={<FormLabel label='Group Name' required={false} />}
                                                onChange={(e) => {
