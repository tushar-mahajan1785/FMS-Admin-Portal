...
import { actionEditManageGroups, actionEmployeeTypeWiseList, actionManageGroupsDetails, actionRosterData, resetEditManageGroupsResponse, resetEmployeeTypeWiseListResponse, resetManageGroupsDetailsResponse } from '../../../../../store/roster';
...
export default function AddNewMemberGroup({ open, handleClose }) {
    ...
    // store
    const { rosterData, employeeTypeWiseList, editManageGroups, manageGroupsDetails } = useSelector(state => state.rosterStore)
    ...
    */
    useEffect(() => {
        if (open === true) {
            if (rosterData?.roster_group_uuid && rosterData?.roster_group_uuid !== null && branch?.currentBranch?.uuid && branch?.currentBranch?.uuid !== null) {
                dispatch(actionManageGroupsDetails({
                    uuid: rosterData?.roster_group_uuid,
    ...
    }, [branch?.currentBranch, rosterData?.role_type_id, open])
    ...
    useEffect(() => {
        if (manageGroupsDetails && manageGroupsDetails !== null) {
            ...
            if (manageGroupsDetails?.result === true) {
                setValue('role_type_id', manageGroupsDetails?.response?.employees[0]?.role_type_id)
                let objData = Object.assign({}, rosterData)
                objData.employees = manageGroupsDetails?.response?.employees
                objData.assets = manageGroupsDetails?.response?.assets
                objData.role_type_id = manageGroupsDetails?.response?.employees[0]?.role_type_id
                dispatch(actionRosterData(objData))
                setEmployeeTypeWiseListOptions(manageGroupsDetails?.response?.employees)
                setEmployeeTypeWiseListOriginalData(manageGroupsDetails?.response?.employees)
            } else {
        ...
        >
            <Stack sx={{ height: '100%' }} justifyContent={'flex-start'} flexDirection={'column'}>
                <FormHeader
                    ...
                    subtitle="Fill below form to add new member to selected group"
                    actions={[
                        <IconButton
                            onClick={handleClose}
                        >
                ...
                >
                    ...
                    <Button
                        ...
                        disabled={loading}
                    >
                        {loading ? <CircularProgress size={18} sx={{ color: 'white' }} /> : 'Add Members'}
                    </Button>
                </Stack>
            </Stack>
        </Drawer>
