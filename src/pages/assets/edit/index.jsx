...
import { actionAssetCustodianList, actionUpdateAsset, resetAssetCustodianListResponse, resetUpdateAssetResponse, actionMasterAssetType, resetMasterAssetTypeResponse } from "../../../store/asset";
...
export default function EditAsset({ open, objData, toggle }) {
    ...
    // store
    const { updateAsset, assetCustodianList, masterAssetType } = useSelector(state => state.AssetStore)
    ...
    useEffect(() => {
        if (open === true) {
            if (objData && objData !== null) {
                setValue('asset_id', objData?.asset_id && objData?.asset_id !== null ? objData?.asset_id : '')
                setValue('asset_description', objData?.asset_description && objData?.asset_description !== null ? objData?.asset_description : '')
                setValue('type', objData?.type && objData?.type !== null ? objData?.type : '')
                setValue('sub_type', objData?.sub_type && objData?.sub_type !== null ? objData?.sub_type : '')
                setValue('make', objData?.make && objData?.make !== null ? objData?.make : '')
                setValue('model', objData?.model && objData?.model !== null ? objData?.model : '')
                setValue('rating_capacity', objData?.rating_capacity && objData?.rating_capacity !== null ? objData?.rating_capacity : '')
                setValue('vendor_id', objData?.vendor_id && objData?.vendor_id !== null ? objData?.vendor_id : '')
                setValue('serial_no', objData?.serial_no && objData?.serial_no !== null ? objData?.serial_no : '')
                setValue('manufacturing_date', objData?.manufacturing_date && objData?.manufacturing_date !== null ? moment(objData?.manufacturing_date, 'YYYY-MM-DD').format('DD/MM/YYYY') : '')
                setValue('installation_date', objData?.installation_date && objData?.installation_date !== null ? moment(objData?.installation_date, 'YYYY-MM-DD').format('DD/MM/YYYY') : '')
                setValue('commissioning_date', objData?.commissioning_date && objData?.commissioning_date !== null ? moment(objData?.commissioning_date, 'YYYY-MM-DD').format('DD/MM/YYYY') : '')
                setValue('warranty_start_date', objData?.warranty_start_date && objData?.warranty_start_date !== null ? moment(objData?.warranty_start_date, 'YYYY-MM-DD').format('DD/MM/YYYY') : '')
                setValue('warranty_expiry_date', objData?.warranty_expiry_date && objData?.warranty_expiry_date !== null ? moment(objData?.warranty_expiry_date, 'YYYY-MM-DD').format('DD/MM/YYYY') : '')
                setValue('amc_start_date', objData?.amc_start_date && objData?.amc_start_date !== null ? moment(objData?.amc_start_date, 'YYYY-MM-DD').format('DD/MM/YYYY') : '')
                setValue('amc_expiry_date', objData?.amc_expiry_date && objData?.amc_expiry_date !== null ? moment(objData?.amc_expiry_date, 'YYYY-MM-DD').format('DD/MM/YYYY') : '')
                setValue('asset_end_life_selection', objData?.asset_end_life_selection && objData?.asset_end_life_selection !== null ? moment(objData?.asset_end_life_selection, 'YYYY-MM-DD').format('DD/MM/YYYY') : '')
                setValue('asset_end_life_period', objData?.asset_end_life_period && objData?.asset_end_life_period !== null ? moment(objData?.asset_end_life_period, 'YYYY-MM-DD').format('DD/MM/YYYY') : '')
                setValue('asset_owner', objData?.asset_owner && objData?.asset_owner !== null ? objData?.asset_owner : '')
                setValue('asset_custodian', objData?.asset_custodian_id && objData?.asset_custodian_id !== null ? objData?.asset_custodian_id : '')
                setValue('location', objData?.location && objData?.location !== null ? objData?.location : '')
                setAssetStatus(objData?.status)
            }
        }
    }, [open])

    /**
      * Function to set the value for Additional fields as per Configured Additional Fields for Asset
      */
    useEffect(() => {
    ...
    // set asset custodian value as per objData
    useEffect(() => {
        if (assetCustodianMaster && assetCustodianMaster !== null && assetCustodianMaster.length > 0) {
            setValue('asset_custodian', objData?.asset_custodian_id && objData?.asset_custodian_id !== null ? objData?.asset_custodian_id : '')
        }
    ...
    /**
     * useEffect
     * @dependency : updateAsset
     * @type : HANDLE API RESULT
     * @description : Handle the result of update Asset API
     */
    useEffect(() => {
        if (updateAsset && updateAsset !== null) {
            dispatch(resetUpdateAssetResponse())
            if (updateAsset?.result === true) {
                toggle('save')
                reset()
                setLoading(false)
                showSnackbar({ message: updateAsset?.message, severity: "success" })
            } else {
                setLoading(false)
                switch (updateAsset?.status) {
                    case UNAUTHORIZED:
                        logout()
                        break
                    case ERROR:
                        dispatch(resetUpdateAssetResponse())
                        toast.dismiss()
                        showSnackbar({ message: updateAsset?.message, severity: "error" })
                        break
                    case SERVER_ERROR:
                        toast.dismiss()
                        showSnackbar({ message: updateAsset?.message, severity: "error" })
    ...
    const onSubmit = data => {
        ...
        dispatch(actionUpdateAsset(cleanedData))
    };
        ...
        >
            <Stack sx={{ height: '100%' }} justifyContent={'flex-start'} flexDirection={'column'}>
                <FormHeader
                    ...
                    title="Edit Asset"
                    subtitle="Fill below form to update asset"
                    message="Please setup the employee, vendor and asset type before updating asset."
                    actions={[
                        <IconButton
                            onClick={handleClose}
                        >
